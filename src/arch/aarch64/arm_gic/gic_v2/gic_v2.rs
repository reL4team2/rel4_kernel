use spin::Mutex;
use super::{Gic_Dist_Map,Gic_Cpu_Iface_Map};
use super::consts::*;
...
pub fn cpu_iface_init() {
	...
	#[cfg(all(feature = "ARM_HYPERVISOR_SUPPORT",not(feature = "PLAT_QEMU_ARM_VIRT")))]
	{
		GIC_DIST.regs().security[0].set(0xffffffff);
		GIC_DIST.regs().priority[0].set(0x80808080);
	}
	#[cfg(any(not(feature = "ARM_HYPERVISOR_SUPPORT"),feature = "PLAT_QEMU_ARM_VIRT"))]
	{
	...
	let mut i = GIC_CPUIFACE.regs().int_ack.set(0);
	while ((i & IRQ_MASK) != IRQ_NONE){
		GIC_CPUIFACE.regs().eoi.set(0);
		i = GIC_CPUIFACE.regs().int_ack.set(0);
	}
